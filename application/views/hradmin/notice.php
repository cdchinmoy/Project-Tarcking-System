...
<div class="row">
	<div class="col-sm-12">
		<div class="card">
			...
			<h4 class="card-title" style="float:left">General Notices</h4>&nbsp;&nbsp;&nbsp;<button name="add_general" type="button" id="add_general" class="btn hidden-sm-down btn-success add_general" style="font-size:11px;" data-toggle="modal" data-target="#generalModal">Add New</button>
							...
							<?php //$task_list = get_main_task_list_by_id($list->project_id); 
							 ...
							 ?>
								<tr <?php if($general->notification_status ==0){ echo "style='background:#E1E1D2 !important;'"; } ?>>
									...
									<td>
										<button name="general_view" id="general_view" data-nott-id="<?php echo $general->notice_id ?>" data-name-id="<?php echo $general->notice_name ?>" data-msg-id="<?php echo $general->notice_message ?>" data-date-id="<?php echo $date ?>" data-file-id="<?php echo $general->notice_file ?>" data-type-id="General Notice" data-nots-id="<?php echo $general->notice_type ?>" class="g_view btn btn-success" data-toggle="modal" data-target="#viewModal">View</button>
										<a href="<?php echo base_url()."notice/delete_notice/".$general->notice_id; ?>"><button name="delete_general" id="delete_general" class="btn hidden-sm-down btn-danger">Delete</button></a>
									</td>
								</tr>
								<?php
										}
...
<div class="modal fade" id="generalModal" role="dialog" data-keyboard="false" data-backdrop="static">
	<div class="modal-dialog">

	  <!-- Modal content-->
	  <div class="modal-content" id="generalModal_div">
		...
		<form class="feedbackgeneral" id="feedbackgeneral" name="feedbackgeneral" action="javascript:void(0);" method="POST" enctype="multipart/form-data">
			<input type="hidden" name="notice_type" id="notice_type" value="1"/>
				...
				<?php /* ?><label class="col-md-10 col-10 align-self-center">Select Employee<button id="select_all">Select All</button></label>
...
/*$(document).ready(function(e) {
	...
	$.ajax({
		...
		url: "<?php echo base_url()."notice/ajax_get_employee"; ?>",
		success: function(data){
			console.log(data);
